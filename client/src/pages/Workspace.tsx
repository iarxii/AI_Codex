...
const workspace: React.FC = () => {
  ...
  const activeProviderInfo =
    PROVIDER_MAP[activeProvider] || PROVIDER_MAP["local"];
    ...
    try {
      ...
      await fetch(`${baseUrl}${config.API_V1_STR}/workspace/scratchpad`, {
        ...
        headers: {
          ...
          ...(isPremiumSpace && config.COLAB_SECRET
            ? { "X-Codex-Premium-Key": config.COLAB_SECRET }
            : {}),
        },
        body: JSON.stringify({
  ...
  useEffect(() => {
    ...
    const handshakeQuery =
      wsUrl.includes("ngrok") && config.COLAB_SECRET
        ? `&handshake=${config.COLAB_SECRET}`
        : "";

    const socket = new WebSocket(
      `${wsUrl}${config.API_V1_STR}/chat/ws/agent?token=${token}${handshakeQuery}`,
    );
    ws.current = socket;
    ...
    socket.onclose = (event: CloseEvent) => {
      ...
      if (event.code === 4401 || event.code === 401) {
        console.warn(
          "Neural Link: Authentication failed. Redirecting to login.",
        );
    ...
    socket.onmessage = (event) => {
      ...
      if (data.type === "telemetry") {
        ...
        const incoming = Array.isArray(data.tool_calls)
          ? data.tool_calls
          : [data.tool_calls];
              ...
              detail: {
                ...
                content:
                  typeof data.declaration === "string"
                    ? data.declaration
                    : JSON.stringify(data.declaration, null, 2),
                language: "json",
              },
            }),
          );
    ...
    const mHandshakeQuery =
      metricsWsUrl.includes("ngrok") && config.COLAB_SECRET
        ? `&handshake=${config.COLAB_SECRET}`
        : "";
    const mSocket = new WebSocket(
      `${metricsWsUrl}${config.API_V1_STR}/metrics/ws/metrics?token=${token}${mHandshakeQuery}`,
    );
        ...
        if (lastScrolledMsgId.current !== lastMsg.id) {
          setTimeout(() => {
            ...
            if (element && container) {
              ...
              const headerOffset =
                activeSpace?.slug === "trading-space" ? 120 : 76;
  ...
  const loadConversation = async (id: number) => {
    ...
    try {
      ...
      const [response, filesResponse] = await Promise.all([
        fetch(`${baseUrl}${config.API_V1_STR}/conversations/${id}`, {
          headers,
        }),
        fetch(`${baseUrl}${config.API_V1_STR}/workspace/${id}/files`, {
          headers,
        }).catch(() => null),
      ]);
      ...
      if (response.ok) {
        ...
        if (space && (space.recommended_provider || space.recommended_model)) {
          ...
          const prevModel = localStorage.getItem(
            `ai_model_${space.recommended_provider}`,
          );

          if (
            space.recommended_provider &&
            prevProvider !== space.recommended_provider
          ) {
            setProvider(space.recommended_provider as any);
          }
          if (
            space.recommended_model &&
            prevModel !== space.recommended_model
          ) {
            setModel(
              space.recommended_model,
              space.recommended_provider as any,
            );
          }
        ...
        setArtifacts(() => {
          ...
          allArtifacts.forEach((parsed) => {
            if (
              !merged.find(
                (a) =>
                  (a.filePath || a.title) === (parsed.filePath || parsed.title),
              )
            ) {
              merged.push(parsed);
            }
  ...
  const loadWorkspaceFiles = async (id: number) => {
    try {
      ...
      const response = await fetch(
        `${baseUrl}${config.API_V1_STR}/workspace/${id}/files`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
            ...(isPremiumSpace && config.COLAB_SECRET
      ...
      if (response.ok) {
        ...
        setArtifacts((prev) => {
          ...
          prev.forEach((art) => {
            if (
              !art.filePath &&
              !merged.find((m) => m.id === art.id || m.title === art.title)
            ) {
              merged.push(art);
            }
          });
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("workspace-update", handleWorkspaceUpdate);
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("a2ui-artifact", handleA2UIArtifact);
  }, []);
  ...
  const handleNewChat = async () => {
    try {
      ...
      const response = await fetch(endpoint, {
        ...
        body: JSON.stringify({
          title: `Workspace ${new Date().toLocaleTimeString()}`,
        }),
      });
      if (response.ok) {
        const data = await response.json();
  ...
  const handleSend = (e: React.FormEvent, messageOverride?: string) => {
    ...
    try {
      if (!ws.current || ws.current.readyState !== WebSocket.OPEN) {
        throw new Error(
          "Neural link disconnected. Please refresh or check connection.",
        );
      }
      ...
      const payload = {
        ...
        config: modelConfig,
        ...getProviderConnectionParams(activeProvider),
  ...
  const handleCancel = () => {
    if (!ws.current || ws.current.readyState !== WebSocket.OPEN || !loading)
      return;
  ...
  const handleRetry = useCallback(
    (errorMsgId: string, newProvider?: string, newModel?: string) => {
      ...
      try {
        ...
        const payload = {
          ...
          local_backend_mode: getLocalBackendMode(),
          config: modelConfig,
          ...getProviderConnectionParams(providerToUse),
          client_type: "web",
        };
        ws.current.send(JSON.stringify(payload));
      } catch (err: any) {
                ...
                <>
                  ...
                  {isHarnessOpen && (
                    <div
                      ...
                      onClick={() => setIsHarnessOpen(false)}
                    >
                      <div
                      ...
                      >
                        <div className="flex items-center justify-between p-4 border-b border-white/5 safe-area-top">
                          ...
                          <button
                            onClick={() => setIsHarnessOpen(false)}
                            className="p-2.5 rounded-xl bg-white/5 text-gray-400 hover:text-white transition-colors cursor-pointer touch-44"
                          >
                            <svg
                  ...
                  <div
                    className={`hidden lg:flex flex-col border-l border-white/5 bg-[#090A0E] z-10 transition-all duration-300 ${isHarnessCollapsed ? "w-[60px]" : "w-[350px] xl:w-[400px]"}`}
                  >
                    {isHarnessCollapsed ? (
                      <div className="flex flex-col items-center py-4 space-y-6 h-full select-none">
                        ...
                        <div className="flex flex-col items-center space-y-4 flex-1">
                          {activeSpace.slug === "trading-space" ? (
                            <>
                              <div
                                className="w-8 h-8 rounded-lg bg-[#fd3b12]/10 flex items-center justify-center border border-[#fd3b12]/20 text-[#fd3b12] cursor-pointer"
                                onClick={() => setIsHarnessCollapsed(false)}
                              ...
                              <div
                                className="w-8 h-8 rounded-lg bg-amber-500/10 flex items-center justify-center border border-amber-500/20 text-amber-500 cursor-pointer animate-pulse"
                                onClick={() => setIsHarnessCollapsed(false)}
                                title="Risk Monitor"
                              >
                                <span className="text-[10px] font-black font-mono">
                              ...
                              <div
                                className="w-8 h-8 rounded-lg bg-emerald-500/10 flex items-center justify-center border border-emerald-500/20 text-emerald-400 cursor-pointer"
                                onClick={() => setIsHarnessCollapsed(false)}
                                title="Market State"
                              >
                                <span className="text-[10px] font-black font-mono">
                            ...
                            <>
                              ...
                              <div
                                className="w-8 h-8 rounded-lg bg-purple-500/10 flex items-center justify-center border border-purple-500/20 text-purple-400 cursor-pointer"
                                onClick={() => setIsHarnessCollapsed(false)}
                                title="MTP Simulator"
                              >
                                <span className="text-[10px] font-black font-mono">
                            ...
                            <>
                              <div
                                className="w-8 h-8 rounded-lg bg-[#0078D4]/10 flex items-center justify-center border border-[#0078D4]/20 text-[#0078D4] cursor-pointer"
                                onClick={() => setIsHarnessCollapsed(false)}
                                title="Office Data"
                              >
                                <span className="text-[10px] font-black font-mono">
                              ...
                              <div
                                className="w-8 h-8 rounded-lg bg-indigo-500/10 flex items-center justify-center border border-indigo-500/20 text-indigo-400 cursor-pointer"
                                onClick={() => setIsHarnessCollapsed(false)}
                            ...
                            <>
                              <div
                                className="w-8 h-8 rounded-lg bg-[#6366f1]/10 flex items-center justify-center border border-[#6366f1]/20 text-[#6366f1] cursor-pointer"
                                onClick={() => setIsHarnessCollapsed(false)}
