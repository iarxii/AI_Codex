...
const SPECIALIZATIONS = [
  { id: 'conversational', name: 'Conversational', icon: <ChatBubbleLeftRightIcon className="w-3.5 h-3.5" /> },
  { id: 'coding', name: 'Coding', icon: <CodeBracketIcon className="w-3.5 h-3.5" /> },
  { id: 'writing', name: 'Writing', icon: <PencilSquareIcon className="w-3.5 h-3.5" /> },
  { id: 'researcher', name: 'Researcher', icon: <MagnifyingGlassIcon className="w-3.5 h-3.5" /> },
  { id: 'image', name: 'Image Generation', icon: <PhotoIcon className="w-3.5 h-3.5" /> },
];

const TOOLS = [
  { id: 'codebase_search', name: 'Codebase Search', description: 'Search definitions and snippets' },
  { id: 'workspace_writer', name: 'Workspace Writer', description: 'Create and edit files' },
  { id: 'workspace_reader', name: 'Workspace Reader', description: 'Read file contents' },
  { id: 'github_search', name: 'GitHub Search', description: 'Search repositories and code' },
  { id: 'memory_skill', name: 'Memory', description: 'Access project history' },
  { id: 'rag_query', name: 'RAG Query', description: 'Query vector knowledge base' },
  { id: 'shell_exec', name: 'Shell Executor', description: 'Run terminal commands' },
  { id: 'url_reader', name: 'URL Reader', description: 'Fetch and parse web content' },
];
...
const ChatInput: React.FC<ChatInputProps> = ({
  ...
  const [activeSpecialization, setActiveSpecialization] = React.useState(SPECIALIZATIONS[1]); // Default to Coding
  ...
  return (
    <footer className="px-6 pb-5 pt-3 bg-transparent border-t border-black/[0.04] z-20" style={{ overflow: 'visible' }}>
      ...
      <form onSubmit={onSend} className="max-w-4xl mx-auto">
        ...
        <div className="relative bg-[#E2E6EC] border border-black/[0.08] rounded-2xl shadow-md transition-all focus-within:border-[#FF6600]/40 focus-within:shadow-lg focus-within:shadow-[#FF6600]/5">
              ...
              {isSpecializationOpen && (
                <div className="absolute bottom-full left-0 mb-2 w-48 bg-white border border-black/[0.08] rounded-xl shadow-2xl py-2 z-50 animate-in fade-in slide-in-from-bottom-2 duration-200">
                  <div className="px-3 py-1.5 border-b border-black/[0.04] mb-1">
                    <span className="text-[9px] font-black uppercase tracking-widest text-[#7A7D8E]">Specialization</span>
                  </div>
                  {SPECIALIZATIONS.map(spec => (
                    <button
                      ...
                      className={`w-full flex items-center gap-2.5 px-3 py-2 text-[11px] font-medium transition-colors hover:bg-black/[0.03] ${activeSpecialization.id === spec.id ? 'text-[#FF6600]' : 'text-[#4A4D5E]'}`}
                    >
                      {spec.icon}
                      {spec.name}
                    </button>
                  ...
                  <div className="mt-1 pt-1 border-t border-black/[0.04]">
                    <div className="flex items-center justify-between px-3 py-2">
                      <span className="text-[10px] font-black tracking-widest text-[#7A7D8E]">AGENT MODE</span>
                      <button
                        type="button"
            ...
            <div className="relative">
              ...
              {isToolsOpen && (
                <div className="absolute bottom-full left-0 mb-2 w-64 bg-white border border-black/[0.08] rounded-xl shadow-2xl py-2 z-50 animate-in fade-in slide-in-from-bottom-2 duration-200">
                  <div className="px-3 py-1.5 border-b border-black/[0.04] mb-1">
                    <span className="text-[9px] font-black uppercase tracking-widest text-[#7A7D8E]">Capability Matrix</span>
                  </div>
                  <div className="max-h-60 overflow-y-auto">
                    {TOOLS.map(tool => (
                      <button
                        ...
                        type="button"
                        onClick={() => {
                          alert(`Tool ${tool.name} initialization - coming soon!`);
                          setIsToolsOpen(false);
                        }}
                        className="w-full flex flex-col items-start px-3 py-2 text-[11px] transition-colors hover:bg-black/[0.03]"
                      >
                        <span className="font-bold text-[#1A1D2E]">{tool.name}</span>
                        <span className="text-[10px] text-[#7A7D8E] leading-tight">{tool.description}</span>
                      </button>
                    ))}
                  </div>
                </div>
