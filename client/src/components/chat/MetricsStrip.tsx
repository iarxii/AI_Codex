      ...
      <div className="flex items-center gap-4 bg-white/80 backdrop-blur-md border border-black/[0.04] rounded-full px-5 py-1.5 shadow-sm transition-all hover:shadow-md hover:bg-white/95">
        <div className="flex items-center gap-3 text-[10px] uppercase tracking-widest text-[#4A4D5E] font-bold">
          <div className="flex items-center gap-1.5">
            <div className="w-1.5 h-1.5 rounded-full bg-[#fd3b12]" />
            <span className={metrics.cpu > 80 ? "text-red-500" : ""}>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className={metrics.ram > 80 ? "text-red-500" : ""}>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span
              className={
                metrics.igpu && metrics.igpu > 80 ? "text-red-500" : ""
              }
            >
              GPU{" "}
              {metrics.igpu_available
                ? `${Math.round(metrics.igpu || 0)}%`
                : "N/A"}
            </span>
          </div>
          <span className="text-black/10">|</span>
          <div className="flex items-center gap-1.5">
            ...
            <span
              className={metrics.npu && metrics.npu > 80 ? "text-red-500" : ""}
            >
              NPU{" "}
              {metrics.npu_available
                ? `${Math.round(metrics.npu || 0)}%`
                : "N/A"}
            </span>
          </div>
          <span className="text-black/10">|</span>
        ...
        <button
          ...
          className={`flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full transition-all border ${
            isChartExpanded
              ? "bg-[#fd3b12] border-[#fd3b12] text-white"
              : "border-black/[0.08] text-[#7A7D8E] hover:border-[#fd3b12] hover:text-[#fd3b12]"
          }`}
        >
