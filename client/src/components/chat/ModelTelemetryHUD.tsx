...
const ModelTelemetryHUD: React.FC<ModelTelemetryHUDProps> = ({
  telemetry,
  isVisible,
}) => {
  ...
  const totalLatMs = telemetry.latencies.total
    ? (telemetry.latencies.total * 1000).toFixed(0)
    : null;
  ...
  const getCapabilityIcon = (cap: string) => {
    switch (cap.toLowerCase()) {
      ...
      case "multimodal":
        return <CodeBracketSquareIcon className="w-3 h-3" />;
      case "structured":
      ...
      <div className="flex items-center gap-4 bg-[#1A1D2E]/90 backdrop-blur-xl border border-white/10 rounded-full px-5 py-1.5 shadow-2xl transition-all">
        ...
        >
          ...
          <span className="text-[9px] font-bold text-emerald-50/70 uppercase tracking-widest">
            ...
            <span className="text-emerald-400/60 font-medium">TOKENS</span>
          </span>
        </div>
