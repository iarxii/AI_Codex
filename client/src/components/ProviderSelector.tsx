...
import { PROVIDERS, MORE_PROVIDERS, PROVIDER_MAP, getLocalBackendMode, setLocalBackendMode, getVisibleProviderIds } from "./providerMeta";
import type { LocalBackendMode, ProviderId } from "./providerMeta";
...
const ProviderSelector: React.FC<ProviderSelectorProps> = ({
  ...
  const [visibleProviders, setVisibleProviders] = useState<ProviderId[]>(() => getVisibleProviderIds());
  ...
  useEffect(() => {
    ...
    const syncVisibleProviders = () => setVisibleProviders(getVisibleProviderIds());
    window.addEventListener("ai-settings-changed", refreshModels);
    window.addEventListener("ai-settings-changed", syncVisibleProviders);
    return () => {
      window.removeEventListener("ai-settings-changed", refreshModels);
      window.removeEventListener("ai-settings-changed", syncVisibleProviders);
    };
  }, [provider, backendMode, bridgeModels]);
