...
>
> Updated baseline status (2026-09-03, later): the Alibaba ECS routing config is now split into two tiers instead of a single static env var. **Tier 2 "Client-Managed Cloud Connections" (active)** — each user connects their own Alibaba ECS Ollama endpoint (plus Azure AI Foundry and AWS Bedrock) via the CONNEX tab; credentials are stored per-user and resolved by [backend/integrations/cloud_inference.py](../backend/integrations/cloud_inference.py), used only by the premium chat flow. **Tier 1 "Platform-Managed Inference" (disabled)** — the static `ALIBABA_ECS_OLLAMA_URL` env var and `deploy_production.bat --alibaba-endpoint` flag still exist but are inert unless `settings.PLATFORM_MANAGED_INFERENCE_ENABLED` is flipped on, reserved for when AICodex offers inference to paid subscribers directly (see [docs/plans/#76_platform_managed_inference_subscriptions_TODO.md](plans/#76_platform_managed_inference_subscriptions_TODO.md)). The ECS/ACR Terraform below is still valid infrastructure — it's just something a user can point their own CONNEX connection at today, rather than something the platform hosts centrally by default.
